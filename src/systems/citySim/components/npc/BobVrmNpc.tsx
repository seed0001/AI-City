...
const MODEL_URL = "/models/npc/aichris.vrm";
...
/**
 * Bob (VRM) using aichris model + standing loop.
 */
export default function BobVrmNpc({ entityId }: Props) {
  ...
  useEffect(() => {
    ...
    void loader
      ...
      .then((gltfAnim) => {
        ...
        if (!vrmAnimation) {
          console.warn("[BobVrmNpc] No vrmAnimations in", STANDING_VRMA_URL);
          return;
        }
      ...
      .catch((err) => {
        console.warn("[BobVrmNpc] Standing VRMA failed:", err);
      });
